...
import { BarChart2, TrendingUp, Flame, Dumbbell, Clock, Target } from 'lucide-react'
...
function DonutChart ({ segments, size = 120 }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={cx} cy={cy} r={radius} fill="none" stroke="var(--bg-3)" strokeWidth="18" />
      {paths}
    </svg>
  )
}
...
function WeekBar ({ label, value, max, color }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 h-2.5 rounded-full" style={{ background: 'var(--bg-3)' }}>
        <motion.div
...
export function MonthlyStats () {
  ...
  const partSegments = [
    ...
    { label: '하체', pct: stats.partPct.lower, color: '#00D4FF' },
    { label: '전신', pct: stats.partPct.full, color: '#FF6B6B' }
  ].filter((s) => s.pct > 0)

  const levelLabels = { beginner: '초급자', intermediate: '중급자', advanced: '상급자' }
  const levelColors = { beginner: '#00D4FF', intermediate: 'var(--volt)', advanced: '#FF6B6B' }

  return (
    <div className="flex flex-col gap-6">
      {/* 헤더 */}
      <div className="flex items-center gap-2">
        <BarChart2 size={18} style={{ color: 'var(--volt)' }} />
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="font-black text-lg">{selYear}년</span>
        <button onClick={() => setSelYear(y => y + 1)} className="px-3 py-1.5 rounded-lg text-sm cursor-pointer"
          style={{ background: 'var(--bg-3)', border: 'none', color: 'var(--text-secondary)' }}>▶</button>
      </div>
        ...
        {MONTHS.map((m, i) => {
          ...
          return (
            <motion.button
              ...
              className="px-3 py-2 rounded-xl text-xs font-bold cursor-pointer relative"
              style={{
                background: selMonth === i ? 'var(--volt)' : 'var(--bg-3)',
                color: selMonth === i ? '#000' : hasData ? 'var(--text-primary)' : 'var(--text-muted)',
                border: `1.5px solid ${selMonth === i ? 'var(--volt)' : hasData ? 'rgba(223,255,0,0.2)' : 'transparent'}`
              }}
            >
              {m}
              {hasData && selMonth !== i && (
                <span className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full" style={{ background: 'var(--volt)' }} />
              )}
            </motion.button>
          )
        })}
          ...
          {stats.totalDays === 0 ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <StatCard icon={Flame} label="운동한 날" value={stats.totalDays} suffix="일" color="#FF6B6B"
                  sub={`이달 ${new Date(selYear, selMonth + 1, 0).getDate()}일 중`} />
                <StatCard icon={TrendingUp} label="꾸준함 지수" value={stats.consistencyRate} suffix="%" color="var(--volt)"
                  sub={stats.consistencyRate >= 70 ? '🔥 대단해요!' : stats.consistencyRate >= 40 ? '👍 잘하고 있어요' : '💪 더 할 수 있어요'} />
                <StatCard icon={Dumbbell} label="완료 운동" value={stats.doneExercises} suffix="개" color="#00D4FF"
                  sub={`전체 ${stats.totalExercises}개 중`} />
                <StatCard icon={Target} label="완수율" value={stats.completionRate} suffix="%" color="#a29bfe"
                  sub={stats.completionRate >= 80 ? '완벽해요!' : '조금 더 집중해봐요'} />
              </div>

              {/* 부위 분포 도넛 차트 */}
              {stats.totalParts > 0 && (
                <div className="p-5 rounded-2xl" style={{ background: 'var(--bg-3)', border: '1px solid var(--border)' }}>
                  <h4 className="font-bold text-sm mb-4">운동 부위 분포</h4>
                  <div className="flex items-center gap-6">
                    <div className="relative flex-shrink-0">
                      <DonutChart segments={partSegments} size={110} />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-lg font-black" style={{ color: 'var(--volt)' }}>{stats.totalParts}</div>
                          <div className="text-xs" style={{ color: 'var(--text-muted)' }}>총 세션</div>
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col gap-3 flex-1">
                      {[
                        ...
                        { label: '하체', pct: stats.partPct.lower, count: stats.partCount.lower, color: '#00D4FF' },
                        { label: '전신', pct: stats.partPct.full, count: stats.partCount.full, color: '#FF6B6B' }
                      ].map((p) => (
                        <div key={p.label} className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-bold w-10 text-right" style={{ color: p.color }}>
                            {p.pct}% <span style={{ color: 'var(--text-muted)' }}>({p.count})</span>
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              {/* 주차별 운동 횟수 */}
              <div className="p-5 rounded-2xl" style={{ background: 'var(--bg-3)', border: '1px solid var(--border)' }}>
                <h4 className="font-bold text-sm mb-4">주차별 운동 횟수</h4>
                <div className="flex flex-col gap-3">
                  {stats.weekCount.map((cnt, i) => (
                    <WeekBar key={i} label={`${i + 1}주`} value={cnt} max={stats.maxWeek}
                      color={cnt === stats.maxWeek && cnt > 0 ? 'var(--volt)' : 'rgba(223,255,0,0.4)'} />
                  ))}
                </div>
              </div>
              ...
              {Object.values(stats.levelCount).some(v => v > 0) && (
                <div className="p-5 rounded-2xl" style={{ background: 'var(--bg-3)', border: '1px solid var(--border)' }}>
                  <h4 className="font-bold text-sm mb-4">난이도 분포</h4>
                    ...
                    {Object.entries(stats.levelCount).map(([level, count]) => {
                      ...
                      return (
                        <div key={level} className="flex-1 p-3 rounded-xl text-center"
                          style={{ background: `${levelColors[level]}15`, border: `1px solid ${levelColors[level]}40` }}>
                          <div className="text-2xl font-black" style={{ color: levelColors[level] }}>{pct}%</div>
                          <div className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>{levelLabels[level]}</div>
                          <div className="text-xs font-bold" style={{ color: levelColors[level] }}>{count}회</div>
                        </div>
                      )
                    })}
                  </div>
              ...
              {Object.values(stats.timeCount).some(v => v > 0) && (
                <div className="p-5 rounded-2xl" style={{ background: 'var(--bg-3)', border: '1px solid var(--border)' }}>
                  <h4 className="font-bold text-sm mb-4">운동 시간 분포</h4>
                  <div className="flex gap-3">
                    {[30, 60, 90].map((t) => {
                      ...
                      return (
                        <div key={t} className="flex-1 p-3 rounded-xl text-center"
                          ...
                          <div className="text-xl font-black" style={{ color: 'var(--volt)' }}>{pct}%</div>
